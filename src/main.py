...
sys.path.append('/root/autodl-fs/MSRP-IFCN-main')
            ...
            for archive_name in [args.UNIVARIATE_ARCHIVE_NAMES]:
                ...
                for iter in range(args.ITERATIONS):
                    ...
                    for dataset_name in [args.UNIVARIATE_DATASET_NAMES]:
                        ...
                        x_train, y_train, x_test, y_test, y_true, y_true_train, nb_classes = prepare_data(datasets_dict,
                                                                                                          dataset_name)
                        # generate_output_directory
                        output_directory, output_directory_models = generate_output_directory(iter, root_dir,
                                                                                              classifier_name,
                                                                                              archive_name,
                                                                                              dataset_name)
                        # training process
                        history, training_time = fit_classifier(classifier_name, args.EPOCH, args.BATCH_SIZE, args.LR,
                                                                ...
                                                                args.test_split, args.save_best_train_model,
                                                                args.save_best_test_model)
                        print('\t\t\t\t TRAINING DONE')
                ...
                for dataset_name in [args.UNIVARIATE_DATASET_NAMES]:
                    ...
                    if args.read_best_train_model:
                        classier_predict_and_save_results('train', x_test, y_test, nb_classes, network,
                                                          args.model_save_interval, args.EPOCH, dataset_name,
                                                          ITERATIONS_RANGE,
