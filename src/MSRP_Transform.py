...
def MS_RP_trans(x, m, tau, sign_flag, image_size):
    ...
    for mi in range(m):
        if mi == 0:
            ...
            xe = np.concatenate((xe, y[:, tau * mi: N2 + tau * mi, :]),
                                axis=2)  # e.g. cut out the vector (len 2) in pos 1, 5, 9
...
UNIVARIATE_DATASET_NAMES = [['RefrigerationDevices', 128, 3, 4]]
...
for dataset_param in UNIVARIATE_DATASET_NAMES:
    ...
    tau = dataset_param[3]
    # load training data
    train = pd.read_csv(data_file_path + '//archives/UCR_Archive//' + dataset_id + '/' + dataset_id + '_TRAIN.txt',
                        sep='  ', header=None,
                        engine='python')
    train = np.array(train)
    train_x = train[:, 1:]
    # load test data
    test = pd.read_csv(data_file_path + '//archives/UCR_Archive//' + dataset_id + '/' + dataset_id + '_TEST.txt',
                       sep='  ', header=None,
                       engine='python')
    test = np.array(test)
    test_x = test[:, 1:]
