...
class FFRN(nn.Module):
    ...
    def __init__(self, in_channels, out1_channels_1, out1_channels_2, out1_channels_3,
        ...
        super(FFRN, self).__init__()

        self.conv_input = nn.Conv2d(in_channels, out1_channels_2, kernel_size=3, stride=1, padding=1, bias=True)

        self.module1 = Inception_module(out1_channels_2, out1_channels_1, out1_channels_2,
                                        out1_channels_3, out1_channels_4)

        in_channels_module2 = 64
        self.module2 = Inception_module(in_channels_module2, out2_channels_1, out2_channels_2,
                                        out2_channels_3, out2_channels_4)

        in_channels_module3 = 64
        self.module3 = Inception_module(in_channels_module3, out1_channels_1, out1_channels_2,
                                        out1_channels_3, out1_channels_4)

        self.bottle = nn.Conv2d(out1_channels_2 * 4, out1_channels_2, kernel_size=1, stride=1, padding=0, bias=True)

        self.conv = BasicConv2d(out1_channels_2, out1_channels_2, 3)
    ...
    def forward(self, x):
        ...
        y = self.global_ave_pooling(y).squeeze()
...
class Inception_module(nn.Module):
    ...
    def __init__(self, in_channels, out_channels_1, out_channels_2,
        ...
        # in_channels(1), out1_channels_1(16), out1_channels_2(32), out1_channels_3(96),
        # out1_channels_4(16), out2_channels_1(32), out2_channels_2(64), out2_channels_3(192）,
        # out2_channels_4（32）, num_class

        self.conv_3_1 = BasicConv2d(in_channels, out_channels_2, 3)  # 1，32

        self.conv_3_3_1_1 = BasicConv2d(in_channels, out_channels_2, 3)  # 1，96
        self.conv_3_3_1_2 = BasicConv2d(out_channels_2, out_channels_2, 3)  # 96，96

        in_channels_out = out_channels_2 * 2
        self.conv_3_2 = BasicConv2d(in_channels_out, out_channels_2 * 2, 3)  # 1，32

        self.conv_3_3_2_1 = BasicConv2d(in_channels_out, out_channels_2 * 2, 3)  # 1，96
        self.conv_3_3_2_2 = BasicConv2d(out_channels_2 * 2, out_channels_2 * 2, 3)  # 96，96

        self.conv_1_1 = nn.Conv2d(out_channels_2 * 4, 64, kernel_size=1, stride=1, padding=0)

    def forward(self, x):
        branch_1 = self.conv_3_1(x)

        branch_2 = self.conv_3_3_1_1(x)
        branch_2 = self.conv_3_3_1_2(branch_2)

        cat1 = torch.cat([branch_1, branch_2], dim=1)

        branch_3 = self.conv_3_2(cat1)

        branch_4 = self.conv_3_3_2_1(cat1)
        branch_4 = self.conv_3_3_2_2(branch_4)

        cat2 = torch.cat([branch_3, branch_4], dim=1)

        branch_5 = self.conv_1_1(cat2)

        module_outputs = x + branch_5
...
def train_op(classifier_obj, EPOCH, batch_size, LR, train_x, train_y,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.5,
                                                           patience=50,
                                                           min_lr=0.000002, verbose=True)  # factor=0.2, min_lr=0.000002
    ...
    torch.save(classifier_obj.state_dict(), output_directory_last, _use_new_zipfile_serialization=False)
