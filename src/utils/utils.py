...
from src.utils.constants import parse_args

args = parse_args()
DATASET_NAMES = [args.UNIVARIATE_DATASET_NAMES]
from src.utils.constants import create_classifier
import sklearn
...
def read_all_datasets(root_dir, archive_name, RP_archive_name, split_val=False):  # split_val=False说明不从训练集里划分验证集
    datasets_dict = {}
    ...
    for dataset_name in DATASET_NAMES:
        ...
        x_train, y_train = readucr(file_name + '_TRAIN.txt', file_name_RP + '_TRAIN.npy')
        x_test, y_test = readucr(file_name + '_TEST.txt', file_name_RP + '_TEST.npy')
...
def log_history(EPOCH, lr_results, loss_train_results, accuracy_train_results,
    ...
    history = pd.DataFrame(data=np.zeros((EPOCH, 5), dtype=float),  # ====================================dtype=np.float
...
def save_models(net, output_directory_models, loss_train, loss_train_results,
    ...
    if loss_train <= min(loss_train_results):
        ...
        torch.save(net.state_dict(), output_directory_best_train, _use_new_zipfile_serialization=False)
    ...
    if accuracy_test >= max(accuracy_test_results):
        ...
        torch.save(net.state_dict(), output_directory_best_test, _use_new_zipfile_serialization=False)
...
def classier_predict_and_save_results(flag_train_test, x_test, y_test, nb_classes, network,
    ...
    log_cur_dataset_epoch_accs = pd.DataFrame(data=np.zeros((1, len(model_epoch_test_ids)), dtype=float),
                                              # ==========np.float
                                              index=[dataset_name], columns=model_epoch_test_indexes)
    log_cur_dataset_epoch_mean_accs = pd.DataFrame(data=np.zeros((1, len(model_epoch_test_ids)), dtype=float),
                                                   # ===========
                                                   index=[dataset_name], columns=model_epoch_test_indexes)
    ...
    for iter in ITERATIONS_RANGE:
        ...
        for step, model_id in enumerate(model_epoch_test_ids):
            ...
            train_model_directory = saved_model_directory + 'best_' + flag_train_test + '_model_' + str(
                model_id) + '.pkl'
    ...
    for step, model_index in enumerate(model_epoch_test_indexes):
        iter_mean_epoch_prediction = iter_mean_epoches_concat_predictions[
                                     step * y_test.shape[0]: (step + 1) * y_test.shape[0]]
